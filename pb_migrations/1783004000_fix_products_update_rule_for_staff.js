...
migrate((app) => {
  ...
  // Allow cashier/kitchen staff to update records owned by their admin.
  products.updateRule = 'ownerId = @request.auth.id || ownerId = @request.auth.adminId || @request.auth.role = "admin"'
  products.deleteRule = 'ownerId = @request.auth.id || @request.auth.role = "admin"'
  app.save(products)
}, (app) => {
  let products = null
  try {
